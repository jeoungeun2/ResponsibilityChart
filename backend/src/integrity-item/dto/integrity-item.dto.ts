...
import { IsEnum, IsOptional, IsString, ValidateNested, IsUUID } from 'class-validator';
...
export class IntegrityItemDto {
  @ApiProperty({ description: '정직성 항목 ID', format: 'uuid' })
  @IsUUID()
  id: string;
...
// 기존 ReplaceIntegrityItemsDto는 유지 (참고용)
export class ReplaceIntegrityItemsDto {
  @ApiProperty({ type: [IntegrityItemDto] })
  @ValidateNested({ each: true })
  @Type(() => IntegrityItemDto)
  items: IntegrityItemDto[];
}