import { Injectable, NotFoundException, ConflictException } from '@nestjs/common';
import { CreateOrgRegistrationDto } from './dto/create-org-registration.dto';
import { UpdateOrgRegistrationDto } from './dto/update-org-registration.dto';

@Injectable()
export class OrgRegistrationsService {
  constructor() {}

  /**
   * 조직등록 생성
   */
  async create(dto: CreateOrgRegistrationDto) {
    // 임시 더미 데이터 반환
    return {
      id: 'temp-org-reg-id-' + Date.now(),
      ...dto,
      createdAt: new Date(),
      updatedAt: new Date(),
      executive: {
        id: dto.executiveId,
        name: '임시 임원',
        positionLabel: '임시 직책',
        titleLabel: '임시 직위'
      }
    };
  }

  /**
   * 조직등록 단건 조회
   */
  async findOne(id: string) {
    // 임시 더미 데이터 반환
    return {
      id: id,
      executiveId: 'temp-exec-id',
      committeeRole: 'MEMBER',
      meetingFrequency: 'MONTHLY',
      createdAt: new Date(),
      updatedAt: new Date(),
      executive: {
        id: 'temp-exec-id',
        name: '임시 임원',
        positionLabel: '임시 직책',
        titleLabel: '임시 직위'
      }
    };
  }

  /**
   * 모든 조직등록 조회
   */
  async findAll() {
    // 임시 더미 데이터 반환
    return [
      {
        id: 'temp-1',
        executiveId: 'temp-exec-1',
        committeeRole: 'MEMBER',
        meetingFrequency: 'MONTHLY',
        createdAt: new Date(),
        updatedAt: new Date(),
        executive: {
          id: 'temp-exec-1',
          name: '임시 임원 1',
          positionLabel: '임시 직책 1',
          titleLabel: '임시 직위 1'
        }
      },
      {
        id: 'temp-2',
        executiveId: 'temp-exec-2',
        committeeRole: 'CHAIR',
        meetingFrequency: 'WEEKLY',
        createdAt: new Date(),
        updatedAt: new Date(),
        executive: {
          id: 'temp-exec-2',
          name: '임시 임원 2',
          positionLabel: '임시 직책 2',
          titleLabel: '임시 직위 2'
        }
      }
    ];
  }

  /**
   * 특정 임원의 조직등록 조회
   */
  async findByExecutiveId(executiveId: string) {
    // 임시 더미 데이터 반환
    return {
      id: 'temp-org-reg-id',
      executiveId: executiveId,
      committeeRole: 'MEMBER',
      meetingFrequency: 'MONTHLY',
      createdAt: new Date(),
      updatedAt: new Date(),
      executive: {
        id: executiveId,
        name: '임시 임원',
        positionLabel: '임시 직책',
        titleLabel: '임시 직위'
      }
    };
  }

  /**
   * 조직등록 수정
   */
  async update(id: string, dto: UpdateOrgRegistrationDto) {
    // 임시 더미 데이터 반환
    return {
      id: id,
      executiveId: dto.executiveId || 'temp-exec-id',
      committeeRole: dto.committeeRole || 'MEMBER',
      meetingFrequency: dto.meetingFrequency || 'MONTHLY',
      createdAt: new Date(),
      updatedAt: new Date(),
      executive: {
        id: dto.executiveId || 'temp-exec-id',
        name: '임시 임원',
        positionLabel: '임시 직책',
        titleLabel: '임시 직위'
      }
    };
  }

  /**
   * 조직등록 삭제
   */
  async remove(id: string) {
    // 임시로 아무것도 하지 않음
    console.log(`조직등록 ${id} 삭제 요청됨`);
    return { id };
  }

  /**
   * 특정 임원의 조직등록 삭제
   */
  async removeByExecutiveId(executiveId: string) {
    // 임시로 아무것도 하지 않음
    console.log(`임원 ${executiveId}의 조직등록 삭제 요청됨`);
    return { executiveId };
  }
}
